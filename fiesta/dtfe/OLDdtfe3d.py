...
class Delaunay3D:
    ...
    def _extent(self):
        """ "
        Calculates and stores minimum and maximum of input points in the variable extent.
        """
    ...
    def set_buffer(self, boxsize, buffer_length, buffer_val=0.0, buffer_mass=None):
        ...
        self.usebuffer = True
        self.useperiodic = False
        x_buffer, y_buffer, z_buffer = boundary.buffer_random_3D(
            self.npart, self.boxsize, self.buffer_length
        )
        if len(self.points[0]) == 4:
            points_buffer = coords.coord2points(
                [x_buffer, y_buffer, z_buffer, buffer_val * np.ones(len(x_buffer))]
            )
        else:
            points_buffer = coords.coord2points(
                [
    ...
    def set_periodic(self, boxsize, buffer_length):
        ...
        self.boxsize = boxsize
        self.buffer_length = buffer_length
        points_periodic = boundary.buffer_periodic_3D(
    ...
    def get_dens(self):
        ...
        if len(self.points[0]) == 4:
            self.points_dens = 1.0 / point_volume
        else:
    ...
    def estimate(self, x, y, z):
        ...
        f_est = src.delaunay_estimate_3d(
            simplices, x, y, z, self.x0, self.y0, self.z0, self.f0, self.delf0
        )
        return f_est
