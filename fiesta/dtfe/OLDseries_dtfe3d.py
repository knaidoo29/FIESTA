...
class SeriesDelaunay3D:
    ...
    def set_buffer(self, boxsize, buffer_length):
        ...
        self.usebuffer = True
        self.useperiodic = False
        x_buffer, y_buffer, z_buffer = boundary.buffer_random_particles_3d(
    ...
    def set_periodic(self, boxsize, buffer_length):
        ...
        self.boxsize = boxsize
        self.buffer_length = buffer_length
        x_periodic, y_periodic, z_periodic = boundary.buffer_periodic_particles_3d(
            self.points[:, 0],
            self.points[:, 1],
            self.points[:, 2],
            self.boxsize,
    ...
    def estimate(self, x_est, y_est, z_est, split=2):
        ...
        split_edges = np.linspace(0.0, self.boxsize, split + 1)
        ...
        for i in range(0, len(x_split)):
            xcond_data = np.where(
                (x_data >= x_split[i] - dx_data) & (x_data < x_split[i] + dx_data)
            )[0]
            for j in range(0, len(y_split)):
                ycond_data = np.where(
                    (y_data[xcond_data] >= y_split[j] - dx_data)
                    & (y_data[xcond_data] < y_split[j] + dx_data)
                )[0]
                for k in range(0, len(z_split)):
                    zcond_data = np.where(
                        (z_data[xcond_data[ycond_data]] >= z_split[k] - dx_data)
                        & (z_data[xcond_data[ycond_data]] < z_split[k] + dx_data)
                    )[0]
                    ...
                    cond_est = np.where(
                        (x_est >= x_split[i] - dx_est)
                        & (x_est < x_split[i] + dx_est)
                        & (y_est >= y_split[i] - dx_est)
                        & (y_est < y_split[i] + dx_est)
                        & (z_est >= z_split[i] - dx_est)
                        & (z_est < z_split[i] + dx_est)
                    )[0]
                    if len(cond_est) > 0:
