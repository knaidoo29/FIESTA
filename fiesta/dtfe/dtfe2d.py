...
class Delaunay2D:
        ...
        if len(cond) != 0:  # pragma: no cover
            ...
            for idx in cond:
                idxnearest = np.argmin(
                    (self.x - self.x[idx]) ** 2.0 + (self.y - self.y[idx]) ** 2.0
                )
    ...
    def get_circumcircles(self) -> None:
        ...
        self.circumcircles[cond, 2] = np.sqrt(
    ...
    def determine_delaunay_boundary(self) -> None:
        ...
        cond2 = np.where(
            (point_type0 == -1.0) | (point_type1 == -1.0) | (point_type2 == -1.0)
        )[0]
    ...
    def get_dens(self) -> None:
        ...
        point_area = src.sum_delaunay_area_4_points_2d(
            self.delaunay_area, del_vert0, del_vert1, del_vert2, len(self.points)
        )
    ...
    def estimate(
        ...
        f_est[cond] = src.delaunay_estimate_2d(
            simplices_idx[cond], x[cond], y[cond], self.x0, self.y0, self.f0, self.delf0
        )
        return f_est
