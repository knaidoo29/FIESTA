...
class Delaunay3D:
    ...
    def _check_delaunay(self) -> None:
        ...
        mask[idx] = 1.0
        cond = np.where(mask != 1.0)[0]
        if len(cond) != 0:  # pragma: no cover
            if self.verbose:
                print(
                    "Delaunay computation merged some points due to floating type precision, weights for particles in the DTFE are being adjusted to account for this."
                )
            for idx in cond:
                idxnearest = np.argmin(
                    (self.x - self.x[idx]) ** 2.0
                    + (self.y - self.y[idx]) ** 2.0
                    + (self.z - self.z[idx]) ** 2.0
                )
    ...
    def get_circumcircles(self) -> None:
        ...
        """
        x0, y0, z0 = (
            self.x[self.simplices[:, 0]],
            self.y[self.simplices[:, 0]],
            self.z[self.simplices[:, 0]],
        )
        x1, y1, z1 = (
            self.x[self.simplices[:, 1]],
            self.y[self.simplices[:, 1]],
            self.z[self.simplices[:, 1]],
        )
        x2, y2, z2 = (
            self.x[self.simplices[:, 2]],
            self.y[self.simplices[:, 2]],
            self.z[self.simplices[:, 2]],
        )
        x3, y3, z3 = (
            self.x[self.simplices[:, 3]],
            self.y[self.simplices[:, 3]],
            self.z[self.simplices[:, 3]],
        )
        one = np.ones(len(x0))
        D = 2 * maths.det4by4(
            x0, y0, z0, one, x1, y1, z1, one, x2, y2, z2, one, x3, y3, z3, one
        )
        cond = np.where(D != 0.0)[0]
        A0 = x0**2 + y0**2 + z0**2
        A1 = x1**2 + y1**2 + z1**2
        A2 = x2**2 + y2**2 + z2**2
        A3 = x3**2 + y3**2 + z3**2
        self.circumcircles = np.nan * np.ones((len(x0), 4))
        self.circumcircles[cond, 0] = (
            maths.det4by4(
                A0[cond],
                y0[cond],
                z0[cond],
        ...
        self.circumcircles[cond, 3] = np.sqrt(
            (x0[cond] - self.circumcircles[cond, 0]) ** 2
            + (y0[cond] - self.circumcircles[cond, 1]) ** 2
            + (z0[cond] - self.circumcircles[cond, 2]) ** 2
        )

    def get_solid_angle(
        self,
        px0: Union[float, np.ndarray],
        py0: Union[float, np.ndarray],
        pz0: Union[float, np.ndarray],
        ...
        x3: Union[float, np.ndarray],
        y3: Union[float, np.ndarray],
        z3: Union[float, np.ndarray],
    ) -> np.ndarray:
        """
    ...
    def get_solid_angle_total(self) -> None:
        ...
        solid_angle = self.get_solid_angle(
        ...
        solid_angle = self.get_solid_angle(
            x2, y2, z2, x3, y3, z3, x0, y0, z0, x1, y1, z1
        )
        np.add.at(self.solid_angle_total, id2, solid_angle)
        solid_angle = self.get_solid_angle(
            x3, y3, z3, x0, y0, z0, x1, y1, z1, x2, y2, z2
        )
    ...
    def determine_delaunay_boundary(self) -> None:
        ...
        cond = np.where(
            (np.sum(self.cross_boundary, axis=1) != 0.0)
            | (np.isfinite(self.circumcircles[:, 0]) == False)
        )[0]
        self.simptypes[cond] = (
            -1.0
        )  # Circumcircle center is completely outside the boundary
    ...
    def get_volume(self) -> None:
        ...
        self.delaunay_volume = src.delaunay_volume_3d(
            x, y, z, del_vert0, del_vert1, del_vert2, del_vert3
        )
    ...
    def get_dens(self) -> None:
        ...
        del_vert2 = self.simplices[:, 2]
        del_vert3 = self.simplices[:, 3]
        point_volume = src.sum_delaunay_vol_4_points_3d(
            self.delaunay_volume,
            del_vert0,
            del_vert1,
            del_vert2,
            del_vert3,
    ...
    def set_field(self, f: np.ndarray) -> None:
        ...
        self.delf0 = src.get_delf0_3d(
            x, y, z, f, del_vert0, del_vert1, del_vert2, del_vert3
        )
    ...
    def estimate(
        ...
        if allow_border and self.dtfe_mode == "field":
            cond = np.where(
                (simplices_idx != -1) & (self.simptypes[simplices_idx] != -1.0)
            )[0]
        else:
            cond = np.where(
                (simplices_idx != -1) & (self.simptypes[simplices_idx] == 1.0)
            )[0]
        f_est[cond] = src.delaunay_estimate_3d(
            simplices_idx[cond],
            x[cond],
            y[cond],
            z[cond],
...
class DelaunayMerger3D:
    ...
    def add_border(self, border: dict) -> None:
        ...
        if self.dtfe_mode is None:
            ...
            assert (
                self.dtfe_mode == border["dtfe_mode"]
            ), "Border DTFE must match preassigned border DTFEs."
    ...
    def _filter(self, boundary: List[float]) -> None:
        ...
        self.f = self.f[cond]
        cond = np.where(
            (mask4part[self.simplices[:, 0]] == 1.0)
            & (mask4part[self.simplices[:, 1]] == 1.0)
    ...
    def run(self, boundary: List[float], apply_filter: bool = False) -> None:
        ...
        if self.dtfe_mode == "density":
            self.dtfe.get_dens()
            cond = np.where(self.ptypes == 0.0)[0]
        ...
        cond = np.where(
            ...
            & (
                self.pID[self.dtfe.simplices[:, 1]]
                == self.pID[self.dtfe.simplices[:, 2]]
            )
            & (
                self.pID[self.dtfe.simplices[:, 2]]
                == self.pID[self.dtfe.simplices[:, 3]]
            )
            & (self.ptypes[self.dtfe.simplices[:, 0]] == 0.0)
            & (self.ptypes[self.dtfe.simplices[:, 1]] == 0.0)
            & (self.ptypes[self.dtfe.simplices[:, 2]] == 0.0)
            & (self.ptypes[self.dtfe.simplices[:, 3]] == 0.0)
        )[0]
        self.dtfe.simptypes[cond] = -1.0
