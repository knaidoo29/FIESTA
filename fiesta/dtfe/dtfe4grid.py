...
def delaunay_density4grid2D(
    ...
    for i in range(0, len(idx)):
        ...
        pix_xcond = np.where(
            (x2d[pixID] >= xedges[idx[i]]) & (x2d[pixID] < xedges[idx[i] + 1])
        )[0]
        ...
        for j in range(0, len(idy)):
            ...
            ycond = np.where(
                (y[xcond] >= yedges[idy[j]]) & (y[xcond] < yedges[idy[j] + 1])
            )[0]
            ...
            dtfe.setup(
                x[xcond[ycond]], y[xcond[ycond]], boundary, mass=mass[xcond[ycond]]
            )
            ...
            pix_ycond = np.where(
                (y2d[pixID[pix_xcond]] >= yedges[idy[j]])
                & (y2d[pixID[pix_xcond]] < yedges[idy[j] + 1])
            )[0]

            for _dx2d, _dy2d in zip(dx2d, dy2d):
                dtfe_estimate = dtfe.estimate(
                    x2d[pixID[pix_xcond[pix_ycond]]] + _dx2d,
                    y2d[pixID[pix_xcond[pix_ycond]]] + _dy2d,
                )
                cond = np.where(
                    np.isfinite(dtfe_estimate)
                    & (count[pix_xcond[pix_ycond]] != len(dx2d))
                )[0]
                dens[pixID[pix_xcond[pix_ycond[cond]]]] += dtfe_estimate[cond]
                count[pix_xcond[pix_ycond[cond]]] += 1.0
    ...
    if xperiodic == True or yperiodic == True:
        ...
        boundary = [
            xorigin - xbuffer,
            xorigin + xboxsize + xbuffer,
            yorigin - ybuffer,
            yorigin + yboxsize + ybuffer,
        ]
...
def delaunay_density4grid3D(
    ...
    for i in range(0, len(idx)):
        ...
        pix_xcond = np.where(
            (x3d[pixID] >= xedges[idx[i]]) & (x3d[pixID] < xedges[idx[i] + 1])
        )[0]
        ...
        for j in range(0, len(idy)):
            ...
            pix_ycond = np.where(
                (y3d[pixID[pix_xcond]] >= yedges[idy[j]])
                & (y3d[pixID[pix_xcond]] < yedges[idy[j] + 1])
            )[0]
            ...
            for k in range(0, len(idz)):
                ...
                pix_zcond = np.where(
                    (z3d[pixID[pix_xcond[pix_ycond]]] >= zedges[idz[k]])
                    & (z3d[pixID[pix_xcond[pix_ycond]]] < zedges[idz[k] + 1])
                )[0]

                zcond = np.where(
                    (z[xcond[ycond]] >= zedges[idz[k]])
                    & (z[xcond[ycond]] < zedges[idz[k] + 1])
                )[0]
                ...
                for _dx3d, _dy3d, _dz3d in zip(dx3d, dy3d, dz3d):
                    ...
                    cond = np.where(
                        np.isfinite(dtfe_estimate)
                        & (count[pix_xcond[pix_ycond[pix_zcond]]] != len(dx3d))
                    )[0]
                    dens[pixID[pix_xcond[pix_ycond[pix_zcond[cond]]]]] += dtfe_estimate[
                        cond
                    ]
                    count[pix_xcond[pix_ycond[pix_zcond[cond]]]] += 1.0
    ...
    if len(idx) != 1 or len(idy) != 1 or len(idz) != 1:
        ...
        for i in range(0, len(idx)):
            ...
            for j in range(0, len(idy)):
                ...
                for _dx3d, _dy3d, _dz3d in zip(dx3d, dy3d, dz3d):
                    dtfe_estimate = dtfe.estimate(
                        x3d[pixID[cond]] + _dx3d,
                        y3d[pixID[cond]] + _dy3d,
                        z3d[pixID[cond]] + _dz3d,
                    )
                    cond_isfinite = np.where(
        ...
        for j in range(0, len(idy)):
            ...
            for k in range(0, len(idz)):
                ...
                for _dx3d, _dy3d, _dz3d in zip(dx3d, dy3d, dz3d):
                    ...
                    field[
                        pixID[pix_xcond[pix_ycond[pix_zcond[cond]]]]
                    ] += dtfe_estimate[cond]
                    count[pix_xcond[pix_ycond[pix_zcond[cond]]]] += 1.0
