...
subroutine part2grid_pcs_2d(x, y, f, xlength, ylength, xmin, ymin, npart &
  , nxgrid, nygrid, periodx, periody, fgrid)
  ...
  real(kind=dp), intent(in) :: xlength, ylength, xmin, ymin
  real(kind=dp), intent(out) :: fgrid(nxgrid*nygrid)
  integer :: i, j1, j2, xpix(4), ypix(4), pix
  real(kind=dp) :: wx, wy, dx, dy, xp, yp, xg(4), yg(4), fp
    ...
    do j1 = 1, 4
      do j2 = 1, 4
        if ((xpix(j1) .GE. 0) .AND. (xpix(j1) .LT. nxgrid) &
          ...
          call weight_pcs(yp, yg(j2), dy, wy)
...
end subroutine part2grid_pcs_2d
